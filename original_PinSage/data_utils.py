...
def train_test_split_by_time(df, timestamp, user):
    ...
    def train_test_split(df):
        ...
        n_relations = df.shape[0]
        n_train_rel = round(0.6*n_relations)
        n_val_rel = round((n_relations-n_train_rel)/2)
        n_test_rel = n_relations-n_train_rel-n_val_rel
        if n_relations > 4:
            df.iloc[-(n_val_rel+n_test_rel):-n_test_rel, -3] = False
            df.iloc[-(n_val_rel+n_test_rel):-n_test_rel, -2] = True
            df.iloc[-n_test_rel:, -3] = False
            df.iloc[-n_test_rel:, -1] = True
        return df
#    def train_test_split(df):
#        df = df.sort_values([timestamp])
#        if df.shape[0] > 1:
